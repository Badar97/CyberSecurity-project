...
// node 1, websocket
//let web3 = new Web3('ws://localhost:23001');
...
function receipt_handler(receipt) {
	try {
		...
		web3.eth.getTransactionCount(mykeys.keys[1].address).then((value) => { 
			...
			console.log("Using key for account 1: ", mykeys.keys[1]);
			const signPromise = web3.eth.signTransaction(tx, mykeys.keys[1].privateKey);
				...
				sentTx.on("receipt", (receipt) => {
					...
					try {
						...
						// NB this call is not allowed, because we are acting as keys[1].address
						// but the "retrieve" function is only allowed to keys[1].address;
 						let res = myContract2.methods.retrieve().call(function (err, res) { if (err) { console.error("Error calling method:", err) } else { console.log("Call result: ", res); } });
